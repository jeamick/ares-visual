...
class Chart(AresHtml.Html):
  ...
  """
  name, category, callFnc = 'C3', 'Charts', 'plotC3'
  ...
  __reqCss, __reqJs = ['c3', 'datatables', 'datatables-export'], ['c3', 'datatables', 'datatables-export']
  ...
  def convertMarkDownBlock(cls, data, aresObj=None):
    ...
    if aresObj is not None:
      p = aresObj.chart(data[0].split(":")[1].strip(), records, seriesNames=headers[1:], xAxis=headers[0], chartFamily='C3')
      p.addAttr(attr, isPyData=False)
    return ["aresObj.chart('C3', '%s')" % (data[2:-1])]
